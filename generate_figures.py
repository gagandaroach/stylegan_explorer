...
#----------------------------------------------------------------------------
# 
# MCW Generate for HE Slides
#
# methods to help us with the research
#

output_dir = '/code/research/stain/python_flask_stylegan_presenter/static/generated_images'
pkl_path = '/code/research/stain/python_flask_stylegan_presenter/pkls/network-final.pkl'

def mcw():
    with open(pkl_path, 'rb') as pickle_file:
        _G, _D, Gs = pickle.load(pickle_file)
        for i in range(0,10):
            randsrc = (np.random.rand(2)*1000).astype('int')
            randtarg = (np.random.rand(3)*1000).astype('int')
            draw_style_mixing_figure(os.path.join(output_dir, f'mcw_style_mixing_{randsrc}_{randtarg}.png'), Gs, w=1024, h=1024, src_seeds=randsrc, dst_seeds=randtarg, style_ranges=[range(0,4)]*3+[range(4,8)]*2+[range(8,18)])
            
        # results google drive folder 
        # draw_uncurated_result_figure(os.path.join(output_dir, f'mcw_images_seed_{i**3}.png'), Gs, cx=0, cy=0, cw=1024, ch=1024, rows=3, lods=[0,1,2,2,3,3], seed=i**3)
        # draw_noise_detail_figure(os.path.join(output_dir, 'mcw_figure04-noise-detail.png'), Gs, w=1024, h=1024, num_samples=100, seeds=[1157,1012])
        # draw_noise_components_figure(os.path.join(output_dir, 'mcw_figure05-noise-components.png'), Gs, w=1024, h=1024, seeds=[1967,1555], noise_ranges=[range(0, 18), range(0, 0), range(8, 18), range(0, 8)], flips=[1])
        # draw_truncation_trick_figure(os.path.join(output_dir, 'mcw_figure08-truncation-trick.png'), Gs, w=1024, h=1024, seeds=[91,388], psis=[1, 0.7, 0.5, 0, -0.5, -1])

def generate_random_png(seed=42):
    with open(pkl_path, 'rb') as pickle_file:
        _G, _D, Gs = pickle.load(pickle_file)
            # def draw_uncurated_result_figure(png, Gs, cx, cy, cw, ch, rows, lods, seed):
        return _make_png(os.path.join(output_dir, f'random_png_4_lods.png'), Gs, cx=0, cy=0, cw=1024, ch=1024, rows=1, lods=[0], seed=seed)

def _make_png(png_path, Gs, cx, cy, cw, ch, rows = 1, lods = [0], seed = 123, save = False):
    print(png_path)
    # latents = np.random.RandomState(seed).randn(sum(rows * 2**lod for lod in lods), Gs.input_shape[1])
    latents = np.random.RandomState(seed).randn(sum(rows * 2**lod for lod in lods), Gs.input_shape[1])
...
def draw_random_pngs(count=10, seed=123):
    print(f"Drawing {count} random pngs")
    print(f'Output Dir: {output_dir}')
    with open(pkl_path, 'rb') as pickle_file:
        _G, _D, Gs = pickle.load(pickle_file)
        seeds = (np.random.RandomState(seed).rand(count)*1000).astype('int')
        print(f"Using Seeds: {seeds}")
        for i in range(0,count):
            draw_uncurated_result_figure(os.path.join(output_dir, f'random_{i+1}_{seeds[i]}.png'), Gs, cx=0, cy=0, cw=1024, ch=1024, rows=1, lods=[0], seed=seeds[i])
